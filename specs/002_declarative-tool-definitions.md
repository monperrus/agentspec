...
- A parameter in a block may carry `maps_to: <implementation argument name>` when the model-facing name differs from the implementation's argument name. Parsing also yields `param_map`: model-facing name → implementation argument name, one entry per parameter with a `maps_to` marker, in the same shape as a `tool_dispatch` entry's `param_map`. Parameters without the marker are absent from `param_map` (they pass through under their own name).
- A caller can therefore build a working dispatch table from blocks alone: for each scanned implementation, `{<block name>: {"python_function": <implementation name>, "param_map": <block param_map>}}`. For the built-in tools this yields the same `param_map` values as the default tool set; e.g. the `str_replace` block maps `old_str` → the implementation's old-text argument and `new_str` → its new-text argument, so a `str_replace` call with `old_str`/`new_str` dispatched through such a table performs the replacement.
- A caller can convert a parsed tool-spec block into one OpenAI-compatible schema entry: `{"type": "function", "function": {"name": <name>, "description": <description>, "parameters": {"type": "object", "properties": <parameters>, "required": <required>}}}`. Neither the `required` nor the `maps_to` marker ever appears inside a property; properties keep only their `type` and `description`, and the schema uses the model-facing names.
...
- In a tool-spec block, a missing `name` or `description` yields an empty string, and a missing `parameters` section yields no parameters, and a block with no `required: true` marker yields an empty `required` list (converted to `"required": []`). A block with no `maps_to` marker yields an empty `param_map`; a `maps_to` whose value is empty or equal to the parameter's own name adds no entry. A parameter line with no sub-keys yields an empty object for that parameter.
