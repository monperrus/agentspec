...
- The asynchronous coding agent:
  ...
  - While waiting at the prompt, the REPL checks the completion queue every 250 ms. When one or more background commands have finished, it abandons the pending prompt (printing a newline) and starts a new model turn for each completion, one at a time in completion order, with no user input. Completions that arrive during a turn are handled after that turn ends, before the prompt is shown again.
