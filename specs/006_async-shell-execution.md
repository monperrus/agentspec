...
The implementation registry offers three more built-in implementations that let a model run shell commands in the background: start a command, poll it later, and wait a planned number of minutes without busy-polling. When a command finishes almost at once with small output, the output comes back inline in the start result, which saves a polling round-trip. An agent spec refers to them in `tool_dispatch` by the implementation names `t_execute_async`, `t_query_exec` and `t_plan_delay`. A ready-made asynchronous coding agent offers them to the model together with file reading and writing.
...
- Planned wait (`t_plan_delay`, integer argument `when`): blocks for `when` minutes, then returns `Planned wait of <when> minute(s) elapsed. Ready to re-plan.`
...
- The asynchronous coding agent:
  ...
  - Uses an in-memory agent spec with `status: "default"` and structured tool calls, and offers five tools in this order: `execute_shell_command(command)`, `query_tool_exec(tool_exec_id)`, `plan_shell_command(when: integer)`, `read_file(path)`, `write_file(path, content)`. The last two map to the built-in read and write implementations.
  - Appends to the system prompt: "You are an async coding agent. Start shell commands with execute_shell_command — they run in the background. Use query_tool_exec to poll status. Use plan_shell_command(when) to wait N minutes before re-checking long-running commands."
