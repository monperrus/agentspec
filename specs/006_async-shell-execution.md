...
- The asynchronous coding agent:
  ...
  - Each automatic turn prints a dim line `[completion] <first line of the message>` and sends the model this message as the user turn:
    ...
    - Then `stdout: <text>`, where text is the trailing-whitespace-stripped stdout if its capture file is at most 4096 bytes, `(see <stdout path>)` if it is larger or unreadable, and `(empty)` if it is empty.
    - Then `stderr: <text>` under the same size rule, only when stderr is non-blank.
