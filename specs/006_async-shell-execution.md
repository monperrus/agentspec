...
- Start (`t_execute_async`, argument `command` (string, required) and `when` (integer minutes, optional, default 0)):
  ...
  - Waits at most 100 ms for the command to exit, then returns a JSON object with `tool_exec_id`, `pid` (integer OS process id of the spawned shell process), `started_at`, `cwd`, `stdin_localfile`, `stdout_localfile` and `stderr_localfile` (absolute paths), in that key order.
  ...
  - Otherwise, the command keeps running in the background and the object has only the id, `pid`, `started_at`, `cwd` and path fields above.
