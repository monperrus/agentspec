...
- The default spec generated for a `run://` backend is written with `tool_specs` (the four default tools) and `tools` (the built-in read, write, string-replace and shell implementation names, in that order), plus `status: "default"` and structured tool calls. It no longer contains `inferred_tool_schema` or `tool_dispatch`.
