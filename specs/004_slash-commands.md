...
- Tool description given to the model: `Run a slash command. command: one of clear, compact, model, usage, help. For 'model', pass a model-id in args to switch; omit args to list.`
- Parameters: `command` (required string, enum `clear`, `compact`, `model`, `usage`, `help`, description `Slash command to run.`) and `args` (optional string, default empty, description `Optional argument (e.g. model-id for 'model').`).
