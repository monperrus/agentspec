...
- After a `mid_turn` compaction, the rest of the turn continues on the compacted history. Later tool calls, tool results and the final answer are appended to it, and every later model request in the turn (and in later turns) is sent from it. The same holds after overflow recovery.
...
- Triggering prompt size: a caller of manual compaction may optionally supply the server-reported prompt-token count that triggered it (known for `threshold` and `every_turn` triggers). When not supplied, the session's watermark W is used instead. A W of 0 means compaction never triggered, so the size is unknown. The size is only used for user-facing compaction reporting. When it is unknown (as with `/compact` on a fresh session, or overflow recovery after W has been reset), the number is left out of that reporting. Supplying it does not change the watermark or any compaction decision.
...
- Context-window rejection: an error from a model call counts as a context-window rejection when either (a) the client already classified it as one, or (b) its HTTP status is 400 or 413 and its message, or failing that the HTTP error response body carried by the error, mentions a context or token limit. Case-insensitive phrases that count include "context length", "context window", "token limit", "maximum context", "too many tokens", "prompt is too long", "exceeds the model", "request too large", "input too long" and "reduce the length". The status comes, in order, from the error's status code, from the HTTP status the error itself carries (as raised by a plain HTTP transport), or from its response's status code. If none exists, it comes from the message text, in the form `HTTP <nnn>` or `Error code: <nnn>`. The response body is decoded as UTF-8 with invalid bytes replaced; a body that cannot be read is treated as not mentioning a limit. A 429 never counts, and neither does an error with no status or a 400 whose message and body do not mention a limit.
...
- A plain HTTP transport error with status 400 whose message is only `HTTP Error 400: Bad Request`, but whose response body is `{"error":{"message":"Invalid request: Your request exceeded model token limit: 1048576 (requested: 1680796)"}}`, counts as a context-window rejection, so overflow recovery runs. The same error with body `{"error":{"message":"invalid api key"}}` does not.
- Snapshot: the message snapshot `metadata` includes `compaction`, an object with `enabled` (boolean, default `true`), `trigger_tokens` and `target_tokens` (the session's values, or null when unset). It is informational only.
...
- Mid-turn compaction after the first of two tool steps: the second tool step's call and result and the final answer all end up in the session's history, after the summary message, and the final answer is the last message. The request that produces the final answer contains the summary message rather than the old uncompacted messages.
