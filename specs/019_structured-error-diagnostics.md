...
- The `error` event data contains `text`, the four diagnostic fields and `fmt`.
- The log record written for the same error contains `type`, `error` (the same text as the event's `text`), the four diagnostic fields with the same values as the event, `ts`, and `error_kind` when one applies (e.g. `rate_limit`).
