...
- The agent never probes a model itself. If no spec file exists (or a re-probe is forced, which skips the cached files) and a non-empty endpoint is supplied, a default spec is generated, written (2-space indented JSON) to `agent_spec_<safe>.json` in the project root, overwriting any existing file, and used. It has `model`, `endpoint` (the supplied one), `status: "default"`, `behaviour.call_delivery_mode: "structured_tool_calls"`, and the four default tools below as `tool_specs` and `tools`. The agent prints the dimmed notice `Generated default spec at <file name>`.
