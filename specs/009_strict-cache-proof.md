...
By default the agent runs fail-closed on prompt caching: after the first model call of a session, every later call must return explicit server-side cache accounting showing a nonzero cache hit or, failing that, a nonzero cache write. If it does not, the turn is aborted with an error. Models that cannot prove cache reuse are treated as unsupported. Callers and the CLI can opt out.
...
- In strict mode, for each response with a usage block, once the session count is 2 or more:
  ...
  - Cached tokens ≤ 0 and cache-write tokens > 0 → a cache write counts as proof (the first call whose prefix crosses the provider's minimum cacheable size populates the cache; the next call reads it). The turn does not abort: the agent emits a `cache_written` event with `cache_creation_tokens` (the cache-write count), displayed dim as `Prefix cache written this call (<cache_creation_tokens> tokens); the next call will read it back.` with the count formatted with comma thousands separators (e.g. `4,900`), and the response is processed normally.
  - Cached tokens ≤ 0 and cache-write tokens ≤ 0 → the turn aborts with the error `Strict cache mode requires cached_tokens > 0 after the first LLM call, but the server reported no cache hit.`, unless the minimum-cacheable floor below applies.
...
- A response whose only cache field is a cache-write field has proof and 0 cached tokens: it passes with a `cache_written` event if the write count is > 0, and is treated as a miss (floor relaxation or abort) if the write count is 0.
